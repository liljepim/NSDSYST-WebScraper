...
def run_master(urls, time_limit, batch_size, port=5672):
    """Run the master node"""
    cmd = (
        [sys.executable, "-m", "src.distributed.master", "--urls"]
        + urls
        + [
            "--time",
            str(time_limit),
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Distributed Email Scraper with Hierarchical Nodes"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of workers to start (unlimited supported)",
    )
    parser.add_argument(
        "--batch-size", type=int, default=3, help="URLs per batch per worker"
    )
    parser.add_argument("--sub-nodes", type=int, default=2, help="Sub-nodes per worker")
    parser.add_argument("--port", type=int, default=5672, help="Master port")
    ...
    print(
        f"Initial parallel processing capacity: {args.workers * args.sub_nodes} nodes"
    )
    print("=" * 60)
